...
> เป้าหมาย: เป็น "ความจำถาวร" ของโปรเจกต์ ที่ Codex อ่านอัตโนมัติทุก session
...
- `backend/app/providers/` = persisted AI/MCP registry + MCP client + health/tool discovery + capability routing
- `backend/app/market_data/` = read-only MT5/Alpaca watchlist adapters; ห้ามใช้แทนราคา execution/risk
...
```bash
# backend setup: cd backend && pip install -e ".[dev,postgres]" && alembic upgrade head
